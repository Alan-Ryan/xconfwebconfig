...
func TestCacheCRUD(t *testing.T) {
	...
	obj, err := db.GetCachedSimpleDao().GetOne(db.GetDefaultTenantId(), db.TABLE_MODELS, model.ID)
	...
	// test delete
	err = db.GetCachedSimpleDao().DeleteOne(db.GetDefaultTenantId(), db.TABLE_MODELS, model.ID)
	assert.NilError(t, err)

	// entry is not immediatly removed from cache so we check db first
	_, err = db.GetSimpleDao().GetOne(db.GetDefaultTenantId(), db.TABLE_MODELS, model.ID)
	assert.Assert(t, errors.Is(err, gocql.ErrNotFound))
...
func TestCacheCompressingDataCRUD(t *testing.T) {
	...
	// verify record not in cache
	obj, err := db.GetCachedSimpleDao().GetOne(db.GetDefaultTenantId(), db.TABLE_GENERIC_NS_LIST, nl.ID)
	assert.Assert(t, errors.Is(err, gocql.ErrNotFound))
	assert.Assert(t, obj == nil)
	...
	obj, err = db.GetCachedSimpleDao().GetOneFromCacheOnly(db.GetDefaultTenantId(), db.TABLE_GENERIC_NS_LIST, nl.ID)
	assert.Assert(t, errors.Is(err, gocql.ErrNotFound))
	assert.Assert(t, obj == nil)
	...
	err = db.GetCachedSimpleDao().SetOne(db.GetDefaultTenantId(), db.TABLE_GENERIC_NS_LIST, nl.ID, nl)
	assert.NilError(t, err)
	...
	// test delete
	err = db.GetCachedSimpleDao().DeleteOne(db.GetDefaultTenantId(), db.TABLE_GENERIC_NS_LIST, nl.ID)
	assert.NilError(t, err)

	// entry is not immediatly removed from cache so we check db first
	_, err = db.GetCompressingDataDao().GetOne(db.GetDefaultTenantId(), db.TABLE_GENERIC_NS_LIST, nl.ID)
	assert.Assert(t, errors.Is(err, gocql.ErrNotFound))
...
func TestCacheGetAllByKeys(t *testing.T) {
	...
	rowKeys := keys[0:3]
	models, err := db.GetCachedSimpleDao().GetAllByKeys(db.GetDefaultTenantId(), db.TABLE_MODELS, rowKeys)
	assert.NilError(t, err)
...
func TestCacheGetAll(t *testing.T) {
	...
	existingKeys, err := db.GetCachedSimpleDao().GetKeys(db.GetDefaultTenantId(), db.TABLE_MODELS)
	assert.NilError(t, err)
	...
	// test GetKeys
	allKeys, err := db.GetCachedSimpleDao().GetKeys(db.GetDefaultTenantId(), db.TABLE_MODELS)
	assert.NilError(t, err)
	assert.Assert(t, (len(existingKeys)+len(newKeys)) == len(allKeys))

	// test GetAllAsList
	modelList, err := db.GetCachedSimpleDao().GetAllAsList(db.GetDefaultTenantId(), db.TABLE_MODELS, 0)
	assert.NilError(t, err)
	...
	// test GetAllAsMap
	modelMap, err := db.GetCachedSimpleDao().GetAllAsMap(db.GetDefaultTenantId(), db.TABLE_MODELS)
	assert.NilError(t, err)
...
func TestCacheRefresh(t *testing.T) {
	...
	db.GetCacheManager().RefreshAll(db.GetDefaultTenantId())

	keys, err := db.GetCachedSimpleDao().GetKeys(db.GetDefaultTenantId(), db.TABLE_MODELS)
	assert.NilError(t, err)
	assert.Assert(t, len(keys) == 0)

	keys, err = db.GetCachedSimpleDao().GetKeys(db.GetDefaultTenantId(), db.TABLE_ENVIRONMENTS)
	assert.NilError(t, err)
	assert.Assert(t, len(keys) == 0)
	...
	db.GetCacheManager().Refresh(db.GetDefaultTenantId(), db.TABLE_MODELS)

	cacheModelKeys, err := db.GetCachedSimpleDao().GetKeys(db.GetDefaultTenantId(), db.TABLE_MODELS)
	assert.NilError(t, err)
	...
	cacheEnvKeys, err := db.GetCachedSimpleDao().GetKeys(db.GetDefaultTenantId(), db.TABLE_ENVIRONMENTS)
	assert.NilError(t, err)
...
func TestCacheInvalidate(t *testing.T) {
	...
	db.GetCacheManager().Invalidate(db.GetDefaultTenantId(), db.TABLE_MODELS, keyToInvalidate.(string))
	...
	// invalidate all keys
	db.GetCacheManager().InvalidateAll(db.GetDefaultTenantId(), db.TABLE_MODELS)
	time.Sleep(500 * time.Millisecond) // wait for async invalidation to complete
...
func TestCacheChangeNotifier(t *testing.T) {
	...
	case msg := <-testNotifier.ch:
		expectedPrefix := fmt.Sprintf("%s: tenantId=%s, tableName=models,", common.ServerOriginId(), db.GetDefaultTenantId())
		assert.Assert(t, strings.HasPrefix(msg, expectedPrefix))
